...
- **访问控制**：按 IP、国家、省/区域、城市、本地流量、远程流量或全部流量放行/阻断（需配置小甜菜科技 IP 归属地 API Key）。
- **连接记录**：代理状态和访问日志写入 PostgreSQL（UTC）；日志文件使用系统本地时区。
- **客户端自恢复**：断线后按 2s 到 30s 指数退避自动重连，并通过 30s 心跳维护控制通道。
...
1. 启动 PostgreSQL，并准备 `nextunnel-server.toml`（填写 `[ip_location].api_key` 后启动 `nextunnel-server`）。
