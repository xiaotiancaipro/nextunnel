...
func GenerateClientToDir(tlsDir string, listenHost string, outDir string) error {
	...
	if caBlock == nil {
		return fmt.Errorf("tls: invalid CA certificate PEM")
	}
	caCert, err := x509.ParseCertificate(caBlock.Bytes)
	if err != nil {
		return fmt.Errorf("tls: parse CA certificate: %w", err)
	}

	keyBlock, _ := pem.Decode(caKeyPEM)
	if keyBlock == nil {
		return fmt.Errorf("tls: invalid CA key PEM")
	}
	caPriv, err := x509.ParsePKCS1PrivateKey(keyBlock.Bytes)
	if err != nil {
		return fmt.Errorf("tls: parse CA private key: %w", err)
	}

	clientPriv, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		return fmt.Errorf("tls: generate client key: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("tls: create client certificate: %w", err)
	}

	if err := os.MkdirAll(outAbs, 0o755); err != nil {
		return fmt.Errorf("tls: mkdir %q: %w", outAbs, err)
	}
	if err := writePEM(clientCrt, "CERTIFICATE", clientDER, 0o644); err != nil {
		return fmt.Errorf("tls: write %s: %w", FileClientCert, err)
	}
	privDER := x509.MarshalPKCS1PrivateKey(clientPriv)
	if err := writePEM(clientKey, "RSA PRIVATE KEY", privDER, 0o600); err != nil {
