...
func (*IpBlock) New(cmd *cobra.Command, cfg *configs.Configs) (ran bool, err error) {

	if !cmd.Flags().Changed("ip-block") {
		return false, nil
	}

	raw, err := cmd.Flags().GetString("ip-block")
	if err != nil {
		return false, err
	}
	...
	if err := service.UpsertIPStatus(*ip, 0); err != nil {
		return true, err
	}

	_, _ = fmt.Fprintf(cmd.OutOrStdout(), "blocked ip %s\n", *ip)
	return true, nil
